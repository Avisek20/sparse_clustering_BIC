...
def permutation_method_to_select_s(
    data, B=30, num_s=30, n_clusters=1, max_iter=300, n_init=30, tol=1e-15, n_jobs=1
):
    ...
    for s in range(num_s):
        ...
        for iter2 in range(B):
            permuted_data_cost[s] = wkmeans(
                    permuted_data_sets[iter2, :, :], n_clusters=n_clusters, s=svals[s],
                    max_iter=max_iter, n_init=n_init, tol=tol, ret_cost=True
            )
        '''
        # cluster the permuted data sets (parallel)
        permuted_data_cost[s] = np.array(Parallel(n_jobs=n_jobs)(delayed(wkmeans)(
            permuted_data_sets[i,:,:], n_clusters=n_clusters, s=svals[s], max_iter=max_iter, n_init=n_init, tol=tol, ret_cost=True
        ) for i in range(B)))
        '''
    ...
    return labels, w, centers, cost, svals, Gap, selected_idx
...
if __name__ == '__main__':
    ...
    for adataset in range(len(datasets)):
        ...
        for i_runs in range(n_runs):
            mem, w, centers, cost, svals, Gap, selected_idx = permutation_method_to_select_s(
                X, B=B, num_s=n_s, n_clusters=k, max_iter=max_iter, n_init=n_init, tol=tol, n_jobs=n_jobs
            )
